...
FROM SHIPMENT s 
where s.created_date >= '2022-01-01' AND s.created_date < '2022-02-01' And s.status_id = 'SHIPMENT_SHIPPED';
...
from shipment s join shipment_route_segment srs using(shipment_id);
...
from shipment s join order_item_ship_group oisg on oisg.order_id = s.primary_order_id AND s.primary_ship_group_seq_id = oisg.ship_group_seq_id
...
SELECT s.primary_order_id as ORDER_ID,
	...
	s.origin_facility_id as FACILITY_ID,
	s.estimated_ship_date as SHIPMENT_DATE
from SHIPMENT s join order_item oi on s.primary_order_id = oi.order_id AND s.primary_ship_group_seq_id = oi.ship_group_seq_id
...
from shipment s JOIN order_header oh on s.primary_order_id = oh.order_id
...
JOIN order_status os
     ON os.order_id = s.primary_order_id
